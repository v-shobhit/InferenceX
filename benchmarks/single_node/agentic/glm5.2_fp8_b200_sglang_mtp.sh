...
# ~756 GB of block-quantized e4m3 weights against ~465 GB for GLM-5.2-NVFP4),
# --quantization fp8 in place of modelopt_fp4, and the memory notes that follow
# from the larger resident weights. Everything else is the NVFP4 B200 script
# unchanged so the two precision curves stay comparable.
#
...
# (see the entry comment on glm5.2-fp8-b200-sglang-agentic-mtp); the DEP branch
...
# A non-empty directory is NOT a staged checkpoint. The NVFP4 sibling found
...
if [[ -n "${MODEL_PATH:-}" ]]; then
    if ! checkpoint_is_complete "$MODEL_PATH"; then
        ...
        # the same shared path, so serialize: one cell pulls the ~756 GB
        # checkpoint (141 shards) and the rest wait on it rather than five
        # racing writers. `hf download` resumes into a partially-populated
        # --local-dir, so a metadata-only stub is fine to download on top of.
        mkdir -p "$MODEL_PATH"
...
if require_agentic_kv_offload_backend hicache; then
    ...
    # complete per-token KV. The ratio 0.75 sizes the host pool off the HBM KV
    # pool, which is smaller here than on NVFP4 (see MEM_FRACTION_STATIC), so
    # ratio mode yields a proportionally smaller pinned tier at c1-c8. Use the
    # same 169 GB/rank absolute target pool at c12/c16 as the NVFP4 sibling:
    # that figure is bounded by host DRAM, which the precision does not change.
    #
...
# the EAGLE draft head's verification-batch activations, the extra CUDA-graph
# capture at 4 draft tokens, and GLM-5.2's DSA indexer temporaries; that
# headroom is a fraction of the card and does not depend on the checkpoint.
...
# pool is roughly 55 GB/GPU here against ~91 GB/GPU on NVFP4. HiCache absorbs
# the difference as host spill; if c12/c16 show HBM pressure, raise the
# fraction here (overridable) before touching the concurrency grid.
